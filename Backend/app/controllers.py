from flask import jsonify, request, send_from_directory, current_app, send_file
...
def generate_response(user_input):
    ...
    try:
        ...
        if hasattr(response, 'text') and response.text:
            ...
            if hasattr(candidate, 'content') and candidate.content:
                if hasattr(candidate.content, 'parts') and candidate.content.parts:
                    # Collect all parts if multiple parts exist
                    raw_content = "".join([part.text for part in candidate.content.parts])
                else:
        ...
        print(f"Raw content length: {len(raw_content)}")  # Debug log
        cleaned = clean_response(raw_content)
        print(f"Cleaned content length: {len(cleaned)}")  # Debug log
        return cleaned
...
def heatmap():
    ...
    sns.heatmap(
        standardized_data, annot=True, cmap="RdYlGn_r", fmt=".2f", linewidths=0.5, center=0
    )
    plt.title("Feature Heatmap by Safety Classification", fontsize=18, pad=20)
    plt.xlabel("Features", fontsize=14)
    plt.ylabel("Safety Classification", fontsize=14)
...
def feature_importance_chart():
    ...
    sns.barplot(
        x='Importance', y='Feature', hue='Feature', data=feature_importance, palette="rocket", legend=False
    )
    plt.title("Feature Importance in Determining Safety Status", fontsize=18, pad=20)
